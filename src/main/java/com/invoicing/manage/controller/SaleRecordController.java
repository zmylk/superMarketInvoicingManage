...
import com.invoicing.manage.request.SaleRecordMonMoneyRequestEntity;
...
public class SaleRecordController {
	...
	public ResponseEntity addSaleRecord(SaleRecordEntity brandEntity){
		try {
			...
			brandEntity.setCashierName("李四");
			brandEntity.setIsDelete(1);
			brandEntity.setDirectlyOrgId(1L);
			brandEntity.setHierarchyId("1");
			brandEntity.setCreatorId(1714020306L);
			brandEntity.setUpdatorId(1L);
			brandEntity.setUpdator("李科");
	...
	/**
	 * goToSaleRecordList员工销售业绩
	 * @return 返回类型为 ModelAndView
	 * @exception
	 * @since JDK 1.7
	 */
	@RequestMapping(value = "/monpage/list", method = RequestMethod.GET)
	public ModelAndView goTogetmoneyList(){
		String url="/sale/money/sale_money_list";
		return new ModelAndView(url);
	}

	/**
	 * getSaleRecordEntityList 获取员工销售业绩
	 * @param saleRecordRequestEntity
	 * @return 返回类型为 ResponseEntity
	 * @exception
	 * @since JDK 1.7
	...
	@RequestMapping(value = "/monpage/list", method = RequestMethod.POST)
	@ResponseBody
	public ResponseEntity getmoneyList(SaleRecordMonMoneyRequestEntity saleRecordRequestEntity){
		logger.debug("method [getSaleRecordEntityList] 查询销售记录列表，请求参数："+JSON.toJSONString(saleRecordRequestEntity));
		PageInfo<SaleRecordMonMoneyEntity> pageInfo=new PageInfo<SaleRecordMonMoneyEntity>();
		pageInfo.setPageNo(saleRecordRequestEntity.getPageNo());
		pageInfo.setPageSize(saleRecordRequestEntity.getPageSize());
		Map<String,Object> params=new HashMap<String,Object>();
		PageInfo<SaleRecordMonMoneyEntity> saleRecordList = saleRecordService.getmoneyList(pageInfo, params);
		logger.debug("method [getSaleRecordEntityList] 查询销售记录列表，返回结果为："+JSON.toJSONString(saleRecordList));
		return new SuccessResponseEntity(saleRecordList);
